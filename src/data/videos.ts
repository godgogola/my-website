/**
 * 衛教短影音資料集
 * 集中管理 YouTube 頻道 @godgogola 最新衛教影片與對應衛教文章
 */

export interface VideoItem {
  id: string;             // YouTube Video ID
  title: string;          // 影片標題
  description: string;    // 影片重點衛教簡述
  category: (typeof videoCategories)[number]; // 分類標籤
  duration: string;       // 影片長度
  url: string;            // YouTube 觀看連結
  featured: boolean;      // 是否列為首頁精選（展示於首頁影音專區）
  order: number;          // 排序序號
  articleSlug?: string;   // 【雙態卡片】對應衛教文章 Slug（選填，有填則自動點亮「閱讀專文」按鈕）
  articleTitle?: string;  // 【雙態卡片】對應衛教文章標題（選填）
}

export const videoCategories = [
  '全部',
  '內視鏡檢查',
  '肝膽胰疾病',
  '胃部疾病',
  '腸道疾病',
  '慢性病',
  '高血壓',
  '高血脂',
  '糖尿病',
  '減重',
  '健康飲食',
  '要活就要動',
] as const;

export const videos: VideoItem[] = [
  // --- 2026 最新發布影音 (2026/09/20) ---
  {
    id: 'FidtR3Zteek',
    title: '跑步五大心率區間',
    description: '跑步與耐力訓練量化核心！解析Zone 1至Zone 5的心率儲備、生理供能系統與體感指標，掌握燃脂與強化心肺的最佳訓練區間。',
    category: '要活就要動',
    duration: '1:01',
    url: 'https://www.youtube.com/watch?v=FidtR3Zteek',
    featured: true,
    order: 1,
    articleSlug: '跑步五大心率區間',
    articleTitle: '跑步五大心率區間',
  },
  {
    id: 'PV2xcZVN6No',
    title: '各種會讓腸胃不舒服的食物',
    description: '常脹氣、胃痛或消化不良？從高發酵性FODMAPs、高脂油炸到強酸刺激性飲品，完整剖析引發腸胃不適的地雷食物與生理誘發機制。',
    category: '健康飲食',
    duration: '1:01',
    url: 'https://www.youtube.com/watch?v=PV2xcZVN6No',
    featured: true,
    order: 2,
    articleSlug: '地雷食物各種會讓腸胃不舒服的食物',
    articleTitle: '【地雷食物】各種會讓腸胃不舒服的食物',
  },
  // --- 2026 最新發布影音 (2026/09/17) ---
  {
    id: 'QxOiC07S_5U',
    title: '肥胖症的併發症',
    description: '肥胖不只是體重問題，更是全身性的慢性發炎！解析肥胖引發糖尿病、心血管疾病、脂肪肝及睡眠呼吸中止等系統性併發症。',
    category: '減重',
    duration: '1:00',
    url: 'https://www.youtube.com/watch?v=QxOiC07S_5U',
    featured: true,
    order: 3,
    articleSlug: '肥胖症肥胖症的併發症',
    articleTitle: '【肥胖症】肥胖症的併發症',
  },
  {
    id: 'B075tkOgJsU',
    title: '肥胖症的原因',
    description: '肥胖真的只是意志力不足嗎？深入探討遺傳、荷爾蒙失調、體重設定點（Set Point）重設與現代高糖高油精緻飲食的致胖關鍵。',
    category: '減重',
    duration: '1:00',
    url: 'https://www.youtube.com/watch?v=B075tkOgJsU',
    featured: true,
    order: 4,
    articleSlug: '肥胖症肥胖症的原因',
    articleTitle: '【肥胖症】肥胖症的原因',
  },
  {
    id: 'wX_x4coVRdo',
    title: '使用瘦瘦針的副作用',
    description: '打瘦瘦針想吐又便秘怎麼辦？解析腸胃蠕動延緩成因、飲食緩解技巧、止吐藥選擇禁忌與度過適應期的實用指南。',
    category: '減重',
    duration: '1:00',
    url: 'https://www.youtube.com/watch?v=wX_x4coVRdo',
    featured: false,
    order: 5,
    articleSlug: '瘦瘦針使用瘦瘦針的副作用',
    articleTitle: '【瘦瘦針】使用瘦瘦針的副作用',
  },
  {
    id: 'HAnVmVkq9tA',
    title: '瘦瘦針停藥後復胖的原因',
    description: '為什麼停用瘦瘦針體重會火速反彈？拆解飢餓素暴增的食慾反撲、肌肉流失造成的基礎代謝崩盤與體重防禦機制。',
    category: '減重',
    duration: '0:41',
    url: 'https://www.youtube.com/watch?v=HAnVmVkq9tA',
    featured: false,
    order: 6,
    articleSlug: '瘦瘦針瘦瘦針停藥後復胖的原因',
    articleTitle: '【瘦瘦針】瘦瘦針停藥後復胖的原因',
  },
  {
    id: 'sseuG7b9Weo',
    title: '打嗝',
    description: '打嗝停不下來千萬別輕忽！超過48小時的頑固性打嗝可能是胃食道逆流、橫膈膜神經刺激或嚴重胃部疾病的警訊。',
    category: '胃部疾病',
    duration: '0:41',
    url: 'https://www.youtube.com/watch?v=sseuG7b9Weo',
    featured: false,
    order: 7,
    articleSlug: '打嗝',
    articleTitle: '打嗝',
  },
  {
    id: 'hgCnVzCK2pU',
    title: '【2026 ADA】合法且具實證基礎的減重方法',
    description: '最新2026美國糖尿病學會（ADA）指南權威解析：密集生活行為介入、FDA核准合法減重藥物與代謝手術三大實證途徑。',
    category: '減重',
    duration: '1:00',
    url: 'https://www.youtube.com/watch?v=hgCnVzCK2pU',
    featured: false,
    order: 8,
    articleSlug: '2026-ada合法且具實證基礎的減重方法',
    articleTitle: '【2026 ADA】合法且具實證基礎的減重方法',
  },
  {
    id: 'nm3keREnygc',
    title: '【2026 ADA】糖尿病照護 心血管 腎臟 代謝（CKM）綜合風險管理',
    description: '2026 ADA 糖尿病照護標準大翻轉！從單純控糖走向「心血管-腎臟-代謝（CKM）」全面器官保護與體重核心管理。',
    category: '糖尿病',
    duration: '0:41',
    url: 'https://www.youtube.com/watch?v=nm3keREnygc',
    featured: false,
    order: 9,
    articleSlug: '2026-ada-糖尿病照護_心血管-腎臟-代謝ckm綜合風險管理',
    articleTitle: '【2026 ADA】 糖尿病照護_心血管-腎臟-代謝（CKM）綜合風險管理',
  },
  {
    id: 'ejbWIapWkOg',
    title: '高尿酸地雷食物',
    description: '痛風不能只怪豆類！果糖手搖飲與啤酒才是飆高尿酸的真正元兇，盤點飲食禁忌與降尿酸實證原則。',
    category: '健康飲食',
    duration: '0:41',
    url: 'https://www.youtube.com/watch?v=ejbWIapWkOg',
    featured: false,
    order: 10,
    articleSlug: '地雷食物高尿酸地雷食物',
    articleTitle: '【地雷食物】高尿酸地雷食物',
  },
  {
    id: 'fzSfjOyjO3I',
    title: '瘦瘦針要如何停藥',
    description: '瘦瘦針達標後該怎麼安全下車？掌握漸進式減量（Tapering）、高蛋白抗肌少與重量訓練，告別停藥暴食與復胖魔咒。',
    category: '減重',
    duration: '0:41',
    url: 'https://www.youtube.com/watch?v=fzSfjOyjO3I',
    featured: false,
    order: 11,
    articleSlug: '瘦瘦針瘦瘦針要如何停藥',
    articleTitle: '【瘦瘦針】瘦瘦針要如何停藥',
  },
// --- 2026 最新發布影音 (2026/09/15) ---
  {
    id: 'YsSXPMT3oHk',
    title: '好膽固醇太高是好事嗎',
    description: '好膽固醇（HDL-C）真的越高越好嗎？解析極端高數值對血管失去保護作用、U型死亡率曲線與心血管健康迷思。',
    category: '高血脂',
    duration: '0:41',
    url: 'https://www.youtube.com/watch?v=YsSXPMT3oHk',
    featured: false,
    order: 12,
    articleSlug: 'hdl好膽固醇太高是好事嗎',
    articleTitle: '【HDL】好膽固醇太高是好事嗎',
  },
  {
    id: '9LZW5TyMJN8',
    title: '高壞膽固醇地雷食物',
    description: '吃什麼會讓壞膽固醇（LDL-C）失控？揪出飽和脂肪大戶與隱形加工地雷飲食，保護心血管遠離動脈粥狀硬化。',
    category: '健康飲食',
    duration: '0:41',
    url: 'https://www.youtube.com/watch?v=9LZW5TyMJN8',
    featured: false,
    order: 13,
    articleSlug: '地雷食物高壞膽固醇地雷食物',
    articleTitle: '【地雷食物】高壞膽固醇地雷食物',
  },
  {
    id: 'Uc2noq__XRI',
    title: '腸躁症地雷食物 FODMAP',
    description: '經常腹脹、腹痛或腹瀉？解析高 FODMAP 發酵碳水化合物對腸道產生的滲透壓與產氣刺激，掌握低腹敏飲食原則。',
    category: '健康飲食',
    duration: '0:41',
    url: 'https://www.youtube.com/watch?v=Uc2noq__XRI',
    featured: false,
    order: 14,
    articleSlug: '地雷食物腸躁症-fodmap',
    articleTitle: '【地雷食物】腸躁症 FODMAP',
  },
  {
    id: 'Pxr4JXd-hRg',
    title: '要如何提昇好的膽固醇',
    description: '血管清道夫 HDL 偏低怎麼辦？彙整有氧運動、阻力訓練、健康油脂選擇與生活調整等 5 大實證生活處方。',
    category: '高血脂',
    duration: '0:41',
    url: 'https://www.youtube.com/watch?v=Pxr4JXd-hRg',
    featured: false,
    order: 15,
    articleSlug: 'hdl要如何提昇好的膽固醇',
    articleTitle: '【HDL】要如何提昇好的膽固醇',
  },
  {
    id: 'wvF-j4jZOxU',
    title: '高三酸甘油脂地雷食物',
    description: '血油過高怎麼吃？盤點精緻糖、含糖飲料、高果糖水果與酒精等常見隱形地雷，防範動脈硬化與急性胰臟炎。',
    category: '健康飲食',
    duration: '0:41',
    url: 'https://www.youtube.com/watch?v=wvF-j4jZOxU',
    featured: false,
    order: 16,
    articleSlug: '地雷食物高三酸甘油脂地雷食物',
    articleTitle: '【地雷食物】高三酸甘油脂地雷食物',
  },
  {
    id: 'QjjcFWusVuo',
    title: '肛門口的搔癢感',
    description: '肛門搔癢越抓越癢？解析內痔黏液滲漏刺激、外痔皮贅清潔不易與過度清洗造成的皮膚致敏成因與護理對策。',
    category: '腸道疾病',
    duration: '0:41',
    url: 'https://www.youtube.com/watch?v=QjjcFWusVuo',
    featured: false,
    order: 17,
    articleSlug: '痔瘡肛門口的搔癢感',
    articleTitle: '【痔瘡】肛門口的搔癢感',
  },
  {
    id: '0_rTHkFiWCg',
    title: '功能性消化不良',
    description: '胃鏡檢查正常卻總是腹脹、早飽、上腹痛？認識腦腸互動異常與內臟高敏感性的功能性消化不良實用照護之道。',
    category: '胃部疾病',
    duration: '0:41',
    url: 'https://www.youtube.com/watch?v=0_rTHkFiWCg',
    featured: false,
    order: 18,
    articleSlug: '功能性消化不良',
    articleTitle: '功能性消化不良',
  },
  {
    id: 'c7jElX1VEbE',
    title: '黑腸症',
    description: '大腸鏡檢查發現腸道有如蛇皮黑斑？深入解析長期服用番瀉葉等蒽醌類瀉藥引起的黑腸症（Melanosis coli）成因與處置。',
    category: '腸道疾病',
    duration: '0:41',
    url: 'https://www.youtube.com/watch?v=c7jElX1VEbE',
    featured: false,
    order: 19,
    articleSlug: '黑腸症',
    articleTitle: '黑腸症',
  },
  // --- 2026 最新發布影音 ---
  {
    id: 'VeS9ab3m4eY',
    title: '高品質大腸鏡',
    description: '大腸鏡檢查怎麼評估品質？深入解析清腸程度、盲腸到達率與腺瘤偵測率（ADR），在最清晰視野下阻斷大腸癌演變。',
    category: '內視鏡檢查',
    duration: '0:41',
    url: 'https://www.youtube.com/watch?v=VeS9ab3m4eY',
    featured: false,
    order: 20,
    articleSlug: '高品質大腸鏡',
    articleTitle: '高品質大腸鏡',
  },
  {
    id: '5QzZgF4Walk',
    title: '胃食道逆流地雷食物',
    description: '火燒心喉嚨異物感好困擾？盤點咖啡、甜食、辛辣與高油等常見誘發逆流的地雷飲食陷阱，教您聰明替換吃出好胃口。',
    category: '健康飲食',
    duration: '0:41',
    url: 'https://www.youtube.com/watch?v=5QzZgF4Walk',
    featured: false,
    order: 21,
    articleSlug: '地雷食物胃食道逆流地雷食物',
    articleTitle: '【地雷食物】胃食道逆流地雷食物',
  },
  {
    id: 'x24UTTkEBLs',
    title: '碳水化合物造成脂肪堆積的機制',
    description: '吃澱粉甜食為什麼特別容易長胖？解析過量醣類轉化為脂肪的體內途徑、胰島素刺激與內臟脂肪堆積的代謝關鍵。',
    category: '減重',
    duration: '1:01',
    url: 'https://www.youtube.com/watch?v=x24UTTkEBLs',
    featured: false,
    order: 22,
    articleSlug: '碳水化合物造成脂肪堆積的機制',
    articleTitle: '碳水化合物造成脂肪堆積的機制',
  },
  {
    id: 'ccNwVrnfOdU',
    title: '【2025 2030年美國人飲食指南】',
    description: '以「真食物」為核心的最新飲食趨勢！解讀 2025-2030 美國飲食指南每日飲食重要改變，遠離超加工食品打造健康體質。',
    category: '健康飲食',
    duration: '1:01',
    url: 'https://www.youtube.com/watch?v=ccNwVrnfOdU',
    featured: false,
    order: 23,
    articleSlug: '2025-2030年美國人飲食指南每日飲食的重要改變',
    articleTitle: '【2025-2030年美國人飲食指南】每日飲食的重要改變',
  },
  {
    id: 'AwR8aWhIF8Q',
    title: '酒精性肝病變',
    description: '長期飲酒如何悄悄摧毀肝臟？從酒精性脂肪肝、酒精性肝炎到肝硬化的三部曲演變，掌握肝臟修復與及早戒酒契機。',
    category: '肝膽胰疾病',
    duration: '0:41',
    url: 'https://www.youtube.com/watch?v=AwR8aWhIF8Q',
    featured: false,
    order: 24,
    articleSlug: '酒精性肝病變',
    articleTitle: '酒精性肝病變',
  },
  {
    id: 'hBuG7IPxWRQ',
    title: '【腸鏡前準備】清腸劑:比較保可淨及耐福力散',
    description: '做大腸鏡該選哪種瀉藥？詳細比對保可淨與耐福力散在口感、飲水量、滲透壓機轉與適合體質上的實用挑選指南。',
    category: '內視鏡檢查',
    duration: '0:41',
    url: 'https://www.youtube.com/watch?v=hBuG7IPxWRQ',
    featured: false,
    order: 25,
    articleSlug: '腸鏡前準備清腸劑比較保可淨及耐福力散',
    articleTitle: '【腸鏡前準備】清腸劑比較(保可淨及耐福力散)',
  },
  {
    id: 'Eo7sb0VvcTs',
    title: '【腸鏡前準備】耐福力散',
    description: '等滲透壓清腸劑耐福力散（耐福利散）正確服用全步驟！如何泡製調配、分次飲用節奏與預防脫水電解質失衡注意要點。',
    category: '內視鏡檢查',
    duration: '0:41',
    url: 'https://www.youtube.com/watch?v=Eo7sb0VvcTs',
    featured: false,
    order: 26,
    articleSlug: '腸鏡前準備耐福利散',
    articleTitle: '【腸鏡前準備】(耐福利散)',
  },
  {
    id: 'z-gNT0nBsZU',
    title: '【腸鏡前準備】保可淨',
    description: '小容量好入口的清腸劑保可淨使用秘笈！掌握兩階段服藥黃金時間、水溫調配要訣與補充足量清流質技巧，確保腸道乾淨。',
    category: '內視鏡檢查',
    duration: '0:41',
    url: 'https://www.youtube.com/watch?v=z-gNT0nBsZU',
    featured: false,
    order: 27,
    articleSlug: '腸鏡前準備保可淨',
    articleTitle: '【腸鏡前準備】(保可淨)',
  },
  {
    id: 'gHcM9xxGFag',
    title: '【2026年健保新制】降血脂藥',
    description: '2026年健保擴大降血脂藥物給付新制！掌握LDL壞膽固醇達標門檻、用藥時機與心血管風險評估關鍵。',
    category: '高血脂',
    duration: '1:01',
    url: 'https://www.youtube.com/watch?v=gHcM9xxGFag',
    featured: false,
    order: 28,
    articleSlug: '2026年健保新制降血脂藥',
    articleTitle: '【2026年健保新制】降血脂藥',
  },
  {
    id: 'GQF4nPRswDc',
    title: '小孩才做選擇，我重訓有氧都要',
    description: '減重消脂該選重訓還是有氧？解析重訓提升基礎代謝與有氧高效燃脂的完美黃金比例組合。',
    category: '減重',
    duration: '0:41',
    url: 'https://www.youtube.com/watch?v=GQF4nPRswDc',
    featured: false,
    order: 29,
    articleSlug: '想減重要有氧還是重訓',
    articleTitle: '想減重，要有氧還是重訓',
  },
  {
    id: '5VG5rwPXSy8',
    title: '新血脂指標【ApoB】【Lpa】【sd LDL】【ApoA1】',
    description: '傳統血脂正常心血管依然亮紅燈？解讀 ApoB、Lp(a)、sdLDL 超壞膽固醇與 ApoA1 四大新一代動脈硬化預測指標。',
    category: '高血脂',
    duration: '1:01',
    url: 'https://www.youtube.com/watch?v=5VG5rwPXSy8',
    featured: false,
    order: 30,
    articleSlug: 'apob-lpa-sdldl-apoa1四大進階脂蛋白指標',
    articleTitle: '【ApoB, Lp(a), sdLDL, ApoA1】四大進階脂蛋白指標',
  },
  {
    id: '1Zto2psWASI',
    title: '內痔',
    description: '排便出血擦拭有鮮血小心內痔！解析內痔與外痔成因、常見分級症狀與微創階梯治療全攻略。',
    category: '腸道疾病',
    duration: '0:41',
    url: 'https://www.youtube.com/watch?v=1Zto2psWASI',
    featured: false,
    order: 31,
    articleSlug: '痔瘡內痔與外痔',
    articleTitle: '【痔瘡】內痔與外痔',
  },
  {
    id: 'IXPgRx1HZjM',
    title: '改善胰島素阻抗',
    description: '肥胖脂肪肝的源頭！搞懂胰島素阻抗成因、早期身體警訊，與透過飲食生活作息逆轉阻抗策略。',
    category: '慢性病',
    duration: '1:01',
    url: 'https://www.youtube.com/watch?v=IXPgRx1HZjM',
    featured: false,
    order: 32,
    articleSlug: '改善胰島素阻抗',
    articleTitle: '改善胰島素阻抗',
  },
  {
    id: '7M9UeRu2KT0',
    title: '高三酸甘油脂',
    description: '抽血血漿混濁如牛奶？高三酸甘油脂血症成因、誘發急性胰臟炎風險與飲食藥物控制處方。',
    category: '高血脂',
    duration: '0:41',
    url: 'https://www.youtube.com/watch?v=7M9UeRu2KT0',
    featured: false,
    order: 33,
    articleSlug: '高三酸甘油脂血症',
    articleTitle: '高三酸甘油脂血症',
  },
  {
    id: 'grV4xrO_VQ8',
    title: '為什麼會有黑綠色大便',
    description: '馬桶驚見墨綠色便便好擔心？解析膽汁氧化、飲食鐵劑影響與腸道蠕動過快原因，掌握就醫觀察要點。',
    category: '腸道疾病',
    duration: '0:41',
    url: 'https://www.youtube.com/watch?v=grV4xrO_VQ8',
    featured: false,
    order: 34,
    articleSlug: '為什麼會有墨綠色大便',
    articleTitle: '為什麼會有墨綠色大便',
  },

  // --- 現有精選影音庫 ---
  {
    id: 'gGoY0kMHbPw',
    title: '一直放屁，是腸子出問題了嗎',
    description: '頻繁排氣連環放屁好尷尬？解析日常放屁成因、產氣食物陷阱與腸道菌發酵機制，掌握何時該就醫檢查腸道健康。',
    category: '腸道疾病',
    duration: '0:41',
    url: 'https://www.youtube.com/watch?v=gGoY0kMHbPw',
    featured: false,
    order: 35,
    articleSlug: '放太多屁是不是腸子有問題',
    articleTitle: '放太多屁是不是腸子有問題',
  },
  {
    id: 'bIe3o8ikO40',
    title: '大腸憩室炎',
    description: '腹痛發燒小心大腸長小囊袋發炎！搞懂大腸憩室成因、左下腹劇痛等典型警訊，與急性期抗生素治療及照護關鍵。',
    category: '腸道疾病',
    duration: '0:41',
    url: 'https://www.youtube.com/watch?v=bIe3o8ikO40',
    featured: false,
    order: 36,
    articleSlug: '大腸憩室炎',
    articleTitle: '大腸憩室炎',
  },
  {
    id: 'qO9JWpIJuoI',
    title: '【2026 ACSM】基礎阻力訓練',
    description: '2026 美國運動醫學會（ACSM）最新指引！專為初學者與長輩打造的入門處方，每週 2 天、先大後小安全防範肌少症。',
    category: '要活就要動',
    duration: '0:41',
    url: 'https://www.youtube.com/watch?v=qO9JWpIJuoI',
    featured: false,
    order: 37,
    articleSlug: '2026-acsm基礎阻力訓練',
    articleTitle: '【2026 ACSM】基礎阻力訓練',
  },
  {
    id: 'TJLqk1pfn7Q',
    title: '代謝異常脂肪肝病變',
    description: '脂肪肝不只是肝臟問題更是全身代謝警訊！最新 MASLD 代謝異常脂肪肝病變正面表列診斷、心血管關聯與逆轉全攻略。',
    category: '肝膽胰疾病',
    duration: '0:41',
    url: 'https://www.youtube.com/watch?v=TJLqk1pfn7Q',
    featured: false,
    order: 38,
    articleSlug: '代謝異常脂肪肝',
    articleTitle: '【代謝異常脂肪肝】',
  },
  {
    id: 'MskD8Z4qclA',
    title: '大腸癌風險因子',
    description: '年輕化的大腸癌如何提早防範？深入剖析超加工食品、肥胖微發炎與環境塑料等高風險因子，把握黃金篩檢防癌時機。',
    category: '腸道疾病',
    duration: '0:41',
    url: 'https://www.youtube.com/watch?v=MskD8Z4qclA',
    featured: false,
    order: 39,
    articleSlug: '大腸癌大腸癌危險因子',
    articleTitle: '【大腸癌】大腸癌危險因子',
  },
  {
    id: 'rcDzG-v3JlA',
    title: '【2026 ACSM】基礎阻力訓練套餐',
    description: '在家也能做的抗老強身處方！遵循 2026 ACSM 指南先大後小、多關節動作優先原則，精選 8 大動作全方位強化全身肌群。',
    category: '要活就要動',
    duration: '1:01',
    url: 'https://www.youtube.com/watch?v=rcDzG-v3JlA',
    featured: false,
    order: 40,
    articleSlug: '2026-acsm基礎阻力訓練套餐',
    articleTitle: '【2026 ACSM】基礎阻力訓練套餐',
  },
  {
    id: '3BRS3MTu3Vg',
    title: '糖尿病診斷',
    description: '血糖超標就是糖尿病嗎？掌握 ADA 最新糖尿病診斷 4 大標準（糖化血色素 HbA1c、空腹血糖與耐糖測試）與篩檢時機。',
    category: '糖尿病',
    duration: '0:31',
    url: 'https://www.youtube.com/watch?v=3BRS3MTu3Vg',
    featured: false,
    order: 41,
    articleSlug: '糖尿病診斷標準',
    articleTitle: '糖尿病診斷標準',
  },
  {
    id: 'UfuklQPtsYA',
    title: '糖尿病併發症',
    description: '血糖失控全身器官都遭殃！認識大血管（心肌梗塞、中風）與微血管（腎病變、視網膜病變、神經病變）併發症預防與定期追蹤關鍵。',
    category: '糖尿病',
    duration: '0:41',
    url: 'https://www.youtube.com/watch?v=UfuklQPtsYA',
    featured: false,
    order: 42,
    articleSlug: '糖尿病併發症',
    articleTitle: '糖尿病併發症',
  },
  {
    id: 'Tjp2DGgcJs4',
    title: '高血壓併發症',
    description: '沉默殺手高血壓！長期血壓過高會重創心、腦、腎與眼底動脈，解析心肌梗塞、主動脈剝離與腎衰竭危險警訊。',
    category: '高血壓',
    duration: '0:41',
    url: 'https://www.youtube.com/watch?v=Tjp2DGgcJs4',
    featured: false,
    order: 43,
    articleSlug: '長期血壓控制不良的併發症',
    articleTitle: '長期血壓控制不良的併發症',
  },
  {
    id: 'gkBta3p1_Jo',
    title: '急性痛風',
    description: '半夜大腳趾關節紅腫熱痛如刀割？急性痛風發作黃金 24 小時抗發炎急救對策，與日常尿酸控制飲食關鍵。',
    category: '慢性病',
    duration: '0:41',
    url: 'https://www.youtube.com/watch?v=gkBta3p1_Jo',
    featured: false,
    order: 44,
    articleSlug: '痛風急性痛風的原因',
    articleTitle: '【痛風】急性痛風的原因',
  },
  {
    id: '8uvg8uMzjeY',
    title: 'S ABCDE 高血壓生活型態調整',
    description: '不吃藥也能降血壓？心臟學會權威認證「S-ABCDE」降壓口訣：限鹽、限酒、減重、戒菸、飲食與持續運動完整指引。',
    category: '高血壓',
    duration: '1:11',
    url: 'https://www.youtube.com/watch?v=8uvg8uMzjeY',
    featured: false,
    order: 45,
    articleSlug: 's-abcde-高血壓生活型態調整',
    articleTitle: 'S-ABCDE 高血壓生活型態調整',
  },
  {
    id: '2a2u2uyCS1U',
    title: '高血壓診斷及測量方法',
    description: '量血壓掌握「722 原則」！連續量 7 天、早晚各 1 次、每次量 2 遍取平均，教你破除白袍高血壓、精確診斷高血壓標準。',
    category: '高血壓',
    duration: '0:41',
    url: 'https://www.youtube.com/watch?v=2a2u2uyCS1U',
    featured: false,
    order: 46,
    articleSlug: '最新高血壓診斷分級與-722-量測準則',
    articleTitle: '最新高血壓診斷分級與 722 量測準則',
  },
  {
    id: 'iFDd1LTER_I',
    title: '急性胰臟炎',
    description: '上腹劇痛穿透後背小心自體消化！認識急性胰臟炎三大導火線、疼痛特徵警訊與臨床緊急處置原則。',
    category: '肝膽胰疾病',
    duration: '0:41',
    url: 'https://www.youtube.com/watch?v=iFDd1LTER_I',
    featured: false,
    order: 47,
    articleSlug: '急性胰臟炎',
    articleTitle: '急性胰臟炎',
  },
  {
    id: '4j84jWlgqlY',
    title: '巴瑞氏食道炎',
    description: '火燒心久治不癒小心食道癌前病變！認識巴瑞特氏食道炎成因、腸化生細胞特徵與定期內視鏡追蹤防癌關鍵。',
    category: '胃部疾病',
    duration: '0:41',
    url: 'https://www.youtube.com/watch?v=4j84jWlgqlY',
    featured: false,
    order: 48,
    articleSlug: '巴瑞氏食道炎',
    articleTitle: '巴瑞氏食道炎',
  },
  {
    id: 'Z63MSpIm87o',
    title: '結腸激躁症',
    description: '一緊張壓力大就肚子痛、腹瀉或便秘？搞懂大腸激躁症（IBS）成因、常見分型與低 FODMAP 飲食日常調理。',
    category: '腸道疾病',
    duration: '0:41',
    url: 'https://www.youtube.com/watch?v=Z63MSpIm87o',
    featured: false,
    order: 49,
    articleSlug: '結腸激躁症',
    articleTitle: '結腸激躁症',
  },
  {
    id: 'GmdlJWoGZEA',
    title: '藥物性食道潰瘍',
    description: '吞藥沒喝夠水小心食道黏膜被灼傷！一顆藥丸引發胸口劇痛與吞嚥困難，認識藥物性食道潰瘍預防與照護重點。',
    category: '胃部疾病',
    duration: '0:41',
    url: 'https://www.youtube.com/watch?v=GmdlJWoGZEA',
    featured: false,
    order: 50,
    articleSlug: '藥物性食道潰瘍',
    articleTitle: '藥物性食道潰瘍',
  },
  {
    id: '1zkY0lUePRo',
    title: '膽結石',
    description: '右上腹悶痛、飯後脹氣小心膽結石！認識膽結石成因、高危險群、症狀警訊與微創手術治療時機。',
    category: '肝膽胰疾病',
    duration: '0:41',
    url: 'https://www.youtube.com/watch?v=1zkY0lUePRo',
    featured: false,
    order: 51,
    articleSlug: '膽結石',
    articleTitle: '膽結石',
  },
  {
    id: 'rW2WJlv-6u8',
    title: '打瘦瘦針也要健康',
    description: '打瘦瘦針快速減重也要守護肌肉與代謝！腸胃專科醫師解析 GLP-1 瘦瘦筆正確用藥原則、常見副作用照護與預防復胖關鍵。',
    category: '減重',
    duration: '1:11',
    url: 'https://www.youtube.com/watch?v=rW2WJlv-6u8',
    featured: false,
    order: 52,
    articleSlug: '瘦瘦針打瘦瘦針也要健康',
    articleTitle: '【瘦瘦針】打瘦瘦針也要健康',
  },
  {
    id: '41WgFILCHtM',
    title: '代謝症候群',
    description: '一分鐘快速掌握代謝症候群 5 大關鍵指標，提早防範心血管與三高慢性病風險。',
    category: '慢性病',
    duration: '0:41',
    url: 'https://www.youtube.com/watch?v=41WgFILCHtM',
    featured: false,
    order: 53,
    articleSlug: '代謝症候群',
    articleTitle: '代謝症候群',
  },
  {
    id: 'kbVjl-HOKr4',
    title: '大腸息肉',
    description: '息肉是大腸癌前身！認識常見大腸息肉種類、癌變機率與無痛內視鏡切除預防關鍵。',
    category: '腸道疾病',
    duration: '0:41',
    url: 'https://www.youtube.com/watch?v=kbVjl-HOKr4',
    featured: false,
    order: 54,
    articleSlug: '大腸息肉',
    articleTitle: '大腸息肉',
  },
  {
    id: 'lywTekvOC1k',
    title: '胃食道逆流',
    description: '火燒心、喉嚨異物感？帶您了解胃食道逆流成因、生活飲食調整與最新整合用藥觀念。',
    category: '胃部疾病',
    duration: '0:51',
    url: 'https://www.youtube.com/watch?v=lywTekvOC1k',
    featured: false,
    order: 55,
    articleSlug: '胃食道逆流',
    articleTitle: '胃食道逆流',
  },
  {
    id: 'w3L4lQW7HKU',
    title: '血糖平穩的好習慣',
    description: '遠離血糖雲霄飛車！腸胃專科醫師分享控糖好習慣、日常進食順序與穩糖關鍵心法。',
    category: '糖尿病',
    duration: '1:11',
    url: 'https://www.youtube.com/watch?v=w3L4lQW7HKU',
    featured: false,
    order: 56,
    articleSlug: '平穩血糖上升的飲食與生活策略',
    articleTitle: '平穩血糖上升的飲食與生活策略',
  },
  {
    id: '-Nfxr0NGDQ0',
    title: '無痛腸胃鏡',
    description: '無痛舒眠內視鏡到底會不會痛？一次了解專業麻醉照護流程、安全性與檢查優勢。',
    category: '內視鏡檢查',
    duration: '0:41',
    url: 'https://www.youtube.com/watch?v=-Nfxr0NGDQ0',
    featured: false,
    order: 57,
    articleSlug: '無痛腸胃鏡',
    articleTitle: '無痛腸胃鏡',
  },
  {
    id: 'omtvd71Inwc',
    title: '上消化道潰瘍',
    description: '胃潰瘍與十二指腸潰瘍有何不同？搞懂疼痛發作時間點、致病因素與黃金治療期。',
    category: '胃部疾病',
    duration: '0:41',
    url: 'https://www.youtube.com/watch?v=omtvd71Inwc',
    featured: false,
    order: 58,
    articleSlug: '上消化道潰瘍',
    articleTitle: '上消化道潰瘍',
  },
  {
    id: 'cBAGeNQeMP8',
    title: '一次做完腸胃篩檢',
    description: '為什麼醫師常建議胃鏡、大腸鏡同時做？一次舒眠準備、省時安心、全消化道完整把關。',
    category: '內視鏡檢查',
    duration: '0:41',
    url: 'https://www.youtube.com/watch?v=cBAGeNQeMP8',
    featured: false,
    order: 59,
    articleSlug: '一次做完腸胃篩檢',
    articleTitle: '一次做完腸胃篩檢',
  },
  {
    id: 'S2QcETFlIxI',
    title: '低渣飲食',
    description: '大腸鏡檢查前最關鍵的清腸環節！低渣飲食原則、地雷食物與常見誤區全攻略。',
    category: '內視鏡檢查',
    duration: '0:41',
    url: 'https://www.youtube.com/watch?v=S2QcETFlIxI',
    featured: false,
    order: 60,
    articleSlug: '腸鏡前準備低渣飲食',
    articleTitle: '【腸鏡前準備】低渣飲食',
  },
  {
    id: 'lxOZ1KRJDDE',
    title: '胃幽門桿菌',
    description: '胃癌與消化道潰瘍的頭號隱形殺手！什麼是幽門螺旋桿菌？為什麼感染務必殺菌？',
    category: '內視鏡檢查',
    duration: '0:41',
    url: 'https://www.youtube.com/watch?v=lxOZ1KRJDDE',
    featured: false,
    order: 61,
    articleSlug: '要如何防治胃癌根除幽門桿菌',
    articleTitle: '要如何防治胃癌，根除幽門桿菌',
  },
  {
    id: 'wCFG_s-ToXA',
    title: '幽門桿菌檢驗方法',
    description: '碳13吹氣試驗、糞便抗原快篩、胃鏡切片怎麼選？各項檢查原理與適合對象評估。',
    category: '胃部疾病',
    duration: '0:41',
    url: 'https://www.youtube.com/watch?v=wCFG_s-ToXA',
    featured: false,
    order: 62,
    articleSlug: '幽門桿菌幽門桿菌檢測',
    articleTitle: '【幽門桿菌】幽門桿菌檢測',
  },
  {
    id: '7yhAnDnFC60',
    title: '急性腸胃炎',
    description: '上吐下瀉虛脫無力？急性腸胃炎黃金照護守則、補充電解質要點與危險就醫警訊。',
    category: '腸道疾病',
    duration: '0:41',
    url: 'https://www.youtube.com/watch?v=7yhAnDnFC60',
    featured: false,
    order: 63,
    articleSlug: '急性腸胃炎',
    articleTitle: '急性腸胃炎',
  },
];

/**
 * 取得 YouTube 高畫質縮圖網址
 */
export function getYouTubeThumbnail(videoId: string): string {
  return `https://i.ytimg.com/vi/${videoId}/hqdefault.jpg`;
}

/**
 * 取得 YouTube 嵌入連結（支援自動播放參數）
 */
export function getYouTubeEmbedUrl(videoId: string, autoplay = true): string {
  return `https://www.youtube.com/embed/${videoId}?rel=0&showinfo=0&autoplay=${autoplay ? 1 : 0}`;
}

/**
 * 取得文章網址（支援個人網站站內跳轉與診所跨站跳轉）
 */
export function getArticleUrl(slug?: string, isEmbed = false): string {
  if (!slug) return '';
  const path = `/posts/${slug}`;
  return isEmbed ? `https://drhuanggi.vercel.app${path}` : path;
}
